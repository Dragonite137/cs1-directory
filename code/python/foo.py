...
Idris is a compiled language; Python is interpreted
Idris uses static type checking; Python uses dynamic type checking
Idris values are immutable whereas Python has mutable state
Idris is a language of expressions; Python is a language of commands
...
are known and checked at compile time.  One cannot compile a program 
with a type error in Idris.  There is therefore no need check for
...
is not done.  You can write, compile, and run code that passes a string
to a function expecting a number, and you won't know there's an error
until you run your program and it issues a runtime error.
'''
